...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${BLUE_SQUARE_API_URL}/api/notifications/global`, {
      method: 'POST',
      headers: {
...
export async function GET(request: NextRequest) {
  try {
    ...
    let url = `${BLUE_SQUARE_API_URL}/api/notifications/global`;
    if (notificationId) {
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: data.error || 'Failed to get notifications' },
        { status: response.status }
      );
    }
